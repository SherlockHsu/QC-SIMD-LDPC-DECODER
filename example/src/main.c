...
int main()
{
	...
	qc_simd_ldpc_decoder_t *qch = (qc_simd_ldpc_decoder_t *)malloc(sizeof(qc_simd_ldpc_decoder_t));
	qc_simd_ldpc_decoder_mem_init(68, 46, 384, qch);

	qc_simd_ldpc_decoder_param_init(BG12, 68, 46, 384, 8448, 9984, qch);
	...
	for (indx_ebn0 = 0; indx_ebn0 < test_size; indx_ebn0++)
	{
		...
		for (indx_block = 0; indx_block < BLOCK_SIZE; indx_block++)
		{
			...
			qc_simd_ldpc_decoder_scb(ldpc_arg->rdmed_llr, I_max, coef, decoder_mode, qch, ldpc_arg->decoded_bits);
#if defined(_MSC_VER)
			QueryPerformanceCounter(&num);
			end = num.QuadPart;
	...
	free_qc_simd_ldpc_decoder_t(qch);
